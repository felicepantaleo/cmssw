		...
		std::vector<std::array<int, 4>> & quadruplets)
{
	...
	std::cout << "numberOfKeysIn1stArena size " << numberOfKeysIn1stArena.size()
			<< std::endl;
	for (size_t i = 0; i < theNumberOfLayers - 1; ++i)
	{
		...
		std::cout << "numberOfKeysIn1stArena[" << i << "]: "
				<< numberOfKeysIn1stArena[i] << std::endl;
	}
	...
	checkCudaError(cudaMemset(foundNtuplets, 0x00, sizeof(GPUSimpleVector<maxNumberOfQuadruplets, GPUSimpleVector<4, int>>)));
	checkLastCudaError();
