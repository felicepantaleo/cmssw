...
void CAHitQuadrupletGenerator::hitNtuplets(
  ...
  for (const auto &regionLayerPairs : regionDoublets) {
    ...
    ca.createAndConnectCells(hitDoublets, region, caThetaCut, caPhiCut,
                             caHardPtCut);
    ...
    for (unsigned int quadId = 0; quadId < numberOfFoundQuadruplets; ++quadId) {
      ...
      if (theComparitor) {
        SeedingHitSet tmpTriplet(
            allCells[foundQuadruplets[quadId][0]].getInnerHit(),
