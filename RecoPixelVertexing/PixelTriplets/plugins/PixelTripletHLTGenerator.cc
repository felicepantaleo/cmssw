					   ...
					   const std::vector<SeedingLayerSetsHits::SeedingLayer>& thirdLayers)
{
  ...
  for (std::size_t ip =0;  ip!=doublets.size(); ip++) {
      ...
      if (barrelLayer)
	{
	  ...
	  minPoint.setDimension(1, regMin.min() - nSigmaRZ * rzError[il]);

	  maxPoint.setDimension(0, prmax );
	  maxPoint.setDimension(1, regMax.max() + nSigmaRZ * rzError[il]);
	}
      else
	{

    	  minPoint.setDimension(0, prmin);
    	  minPoint.setDimension(1, rzRange.min()-regOffset-nSigmaRZ*rzError[il]);
    	  maxPoint.setDimension(0, prmax );
    	  maxPoint.setDimension(1, rzRange.max()+regOffset+nSigmaRZ*rzError[il]);
