...
__host__ __device__ inline double cross2D(const Vector2d& a,
                                          const Vector2d& b)
{
                                                    ...
                                                    const double factor)
{
    ...
    const double sig2 =
        .000225 / p_2 * sqr(1 + 0.038 * log(radlen_eff)) * radlen_eff;
    for (u_int k = 0; k < n; ++k)
    {
        for (u_int l = k; l < n; ++l)
        {
            ...
            {
                scatter_cov_rad(k, l) +=
                    (rad(k) - rad(i)) * (rad(l) - rad(i)) * sig2 / sqr(sin(theta));
                scatter_cov_rad(l, k) = scatter_cov_rad(k, l);
            }
        }
    }
    scatter_cov_rad *= factor;
...
/*!
* The following computing is under MultiScattering case
...
__host__ __device__ inline Matrix2Nd Scatter_cov_rtoxy(
    ...
    const circle_fit& circle, double B, const u_int nn, const double factor)
{
    u_int n = p2D.cols();
    double X = 0.04;
    double theta = atan(fast_fit(3));
...
__host__ __device__ inline Vector2d min_eigen2D(const Matrix2d& A,
                                                double& chi2)
{
...
__host__ __device__ inline circle_fit Circle_fit(
    const Matrix2xNd& hits2D, const Matrix2Nd& hits_cov2D,
    const Vector4d& fast_fit, const VectorNd& rad, const double B,
    const double factor, const bool error = true)
{
#if RFIT_DEBUG
    ...
    if (error)
    {
        ...
        Matrix4d Cvc;  // joint cov matrix of (v0,v1,v2,c)
        {
            ...
            Cvc(3, 3) = c;
            // (v.transpose() * C0 * v) + (C0.cwiseProduct(t0)).sum() +
            // (r0.transpose() * t0 * r0);
        }
                                             ...
                                             const bool error = true)
{
    ...
    // cfr
    // https://indico.cern.ch/event/663159/contributions/2707659/attachments/1517175/2368189/Riemann_fit.pdf
    // Slide 11
    ...
    if (zbn < 4)
    {
        scatterxy = Scatter_cov_rtoxy(p2D, hits, fast_fit, circle, B, zbn, factor);
    }

    MatrixNd x_cov = X_cov(hits_cov, circle, Jxs, error, n, zbn, scatterxy);

    MatrixNd scatter_cov_z = Scatter_cov_z(p2D, fast_fit, circle, B, zbn, factor);
    ...
    if (error)
    {
        ...
        Matrix3d cnc = Matrix3d::Zero();

        cnc.block(0, 0, 2, 2) = cn;
        cnc.block(0, 2, 2, 1) = -cn * r0;
        cnc.block(2, 0, 1, 2) = -r0.transpose() * cn;
        cnc.block(2, 2, 1, 1) = varc;

        /*
    Matrix2x3d J3 = Matrix2x3d::Zero();
    J3(0, 0) = - 1. / v(0);
    J3(0, 1) = v(0) / sqr(v(1));
    J3(1, 0) = - v(0) * c / (v(1) * sqrt(sqr(v(0)) + sqr(v(1))));
    J3(1, 1) = - c * (sqr(v(1)) - (sqr(v(0)) + sqr(v(1)))) / (sqr(v(1)) *
    sqrt(sqr(v(0)) + sqr(v(1)))); J3(1, 2) = -sqrt(sqr(v(0)) + sqr(v(1))) * 1. /
    v(1);
    */
        Matrix<double, 2, 3> J3;
        {
...
inline helix_fit Helix_fit(const Matrix3xNd& hits, const Matrix3Nd& hits_cov,
                           const double B, const bool& error = true)
{
    u_int n = hits.cols();
    ...
    circle_fit circle;
    line_fit line;
    circle = Circle_fit(hits.block(0, 0, 2, n),
                        hits_cov.block(0, 0, 2 * n, 2 * n), fast_fit, rad,
                        B, factor, error);
    line = Line_fit(hits, hits_cov, circle, fast_fit, B, factor, error);
