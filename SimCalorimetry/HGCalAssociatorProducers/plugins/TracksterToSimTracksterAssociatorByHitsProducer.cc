...
TracksterToSimTracksterAssociatorByHitsProducer::TracksterToSimTracksterAssociatorByHitsProducer(const edm::ParameterSet &pset)
    : recoTracksterCollectionToken_(consumes<std::vector<ticl::Trackster>>(pset.getParameter<edm::InputTag>("tracksters"))),
      ...
      simTracksterFromCPCollectionToken_(consumes<std::vector<ticl::Trackster>>(pset.getParameter<edm::InputTag>("simTrackstersFromCP"))),
      hitToTracksterMapToken_(consumes<ticl::AssociationMap<ticl::mapWithFraction>>(pset.getParameter<edm::InputTag>("hitToTracksterMap"))),
      hitToSimTracksterMapToken_(consumes<ticl::AssociationMap<ticl::mapWithFraction>>(pset.getParameter<edm::InputTag>("hitToSimTracksterMap"))),
      hitToSimTracksterFromCPMapToken_(consumes<ticl::AssociationMap<ticl::mapWithFraction>>(pset.getParameter<edm::InputTag>("hitToSimTracksterFromCPMap"))),
      hitToSimClusterMapToken_(consumes<ticl::AssociationMap<ticl::mapWithFraction>>(pset.getParameter<edm::InputTag>("hitToSimClusterMap"))),
      hitToCaloParticleMapToken_(consumes<ticl::AssociationMap<ticl::mapWithFraction>>(pset.getParameter<edm::InputTag>("hitToCaloParticleMap"))),
      tracksterToHitMapToken_(consumes<ticl::AssociationMap<ticl::mapWithFraction>>(pset.getParameter<edm::InputTag>("tracksterToHitMap"))),
      simTracksterToHitMapToken_(consumes<ticl::AssociationMap<ticl::mapWithFraction>>(pset.getParameter<edm::InputTag>("simTracksterToHitMap"))) {
  ...
  produces<ticl::AssociationMap<ticl::mapWithFractionAndScore, std::vector<ticl::Trackster>, std::vector<ticl::Trackster>>>("tracksterToSimTracksterFromCPMap");
  produces<ticl::AssociationMap<ticl::mapWithFractionAndScore, std::vector<ticl::Trackster>, std::vector<ticl::Trackster>>>("simTracksterFromCPToTracksterMap");
}

TracksterToSimTracksterAssociatorByHitsProducer::~TracksterToSimTracksterAssociatorByHitsProducer() {}

void TracksterToSimTracksterAssociatorByHitsProducer::produce(edm::StreamID, edm::Event &iEvent, const edm::EventSetup &iSetup) const {
  using namespace edm;
  ...
  auto tracksterToSimTracksterMap = std::make_unique<ticl::AssociationMap<ticl::mapWithFractionAndScore, std::vector<ticl::Trackster>, std::vector<ticl::Trackster>>>();
  auto tracksterToSimTracksterFromCPMap = std::make_unique<ticl::AssociationMap<ticl::mapWithFractionAndScore, std::vector<ticl::Trackster>, std::vector<ticl::Trackster>>>();

  auto simTracksterToTracksterMap = std::make_unique<ticl::AssociationMap<ticl::mapWithFractionAndScore, std::vector<ticl::Trackster>, std::vector<ticl::Trackster>>>();
  auto simTracksterFromCPToTracksterMap = std::make_unique<ticl::AssociationMap<ticl::mapWithFractionAndScore, std::vector<ticl::Trackster>, std::vector<ticl::Trackster>>>();
  ...
  for (unsigned int tracksterIndex = 0; tracksterIndex < recoTracksters.size(); ++tracksterIndex) {
    ...
    ticl::mapWithFraction hitToAssociatedSimTracksterMap(recoTracksterHitsAndFractions.size());
    std::vector<unsigned int> associatedSimTracksterIndices;
    ticl::mapWithFraction hitToAssociatedSimTracksterFromCPMap(recoTracksterHitsAndFractions.size());
    ...
    const auto& hitToSimTracksterVec = hitToSimTracksterMap[hitIndex];
      for (const auto& [simTracksterIndex, simFraction] : hitToSimTracksterVec) {
        unsigned int simClusterIndex = simTracksters[simTracksterIndex].seedIndex();
        float simClusterFraction = 0;
        auto it = std::find_if(hitToSimClusterMap[hitIndex].begin(), hitToSimClusterMap[hitIndex].end(), [simClusterIndex](const auto& pair) { return pair.first == simClusterIndex; });
        if (it != hitToSimClusterMap[hitIndex].end()) {
          simClusterFraction = it->second;
        }
        hitToAssociatedSimTracksterMap[i].emplace_back(simTracksterIndex, simClusterFraction);
    ...
    const auto& hitToSimTracksterFromCPVec = hitToSimTracksterFromCPMap[hitIndex];
      for (const auto& [simTracksterIndex, simFraction] : hitToSimTracksterFromCPVec) {
        ...
        auto it = std::find_if(hitToCaloParticleMap[hitIndex].begin(), hitToCaloParticleMap[hitIndex].end(), [caloParticleIndex](const auto& pair) { return pair.first == caloParticleIndex; } );
    ...
    associatedSimTracksterIndices.erase(std::unique(associatedSimTracksterIndices.begin(), associatedSimTracksterIndices.end()), associatedSimTracksterIndices.end());

    std::sort(associatedSimTracksterFromCPIndices.begin(), associatedSimTracksterFromCPIndices.end());
    associatedSimTracksterFromCPIndices.erase(std::unique(associatedSimTracksterFromCPIndices.begin(), associatedSimTracksterFromCPIndices.end()), associatedSimTracksterFromCPIndices.end());
    ...
    for (unsigned int i = 0; i < recoTracksterHitsAndFractions.size(); ++i)
    {
      ...
      for ( unsigned int simTracksterIndex : associatedSimTracksterIndices) {
        if (std::find_if(simTracksterVec.begin(), simTracksterVec.end(), [simTracksterIndex](const auto& pair) { return pair.first == simTracksterIndex; } ) == simTracksterVec.end()) {
      ...
      for ( unsigned int simTracksterIndex : associatedSimTracksterFromCPIndices) {
        if (std::find_if(simTracksterFromCPVec.begin(), simTracksterFromCPVec.end(), [simTracksterIndex](const auto& pair) { return pair.first == simTracksterIndex; } ) == simTracksterFromCPVec.end()) {
    ...
    for (unsigned int i = 0; i < recoTracksterHitsAndFractions.size(); ++i) {
      ...
      const auto& recHit = rechitManager[hitIndex];
      float squaredRecoFraction = recoTracksterHitsAndFractions[i].second * recoTracksterHitsAndFractions[i].second;
      float squaredRecHitEnergy = recHit.energy() * recHit.energy();
      float recoSharedEnergy = recHit.energy() * recoTracksterHitsAndFractions[i].second;
      ...
      for (const auto& [simTracksterIndex, simFraction] : simTracksterVec) {
        ...
        float score = invDenominator * squaredRecoFraction * squaredRecHitEnergy;
...
// Reverse mapping: SimTrackster -> RecoTrackster
  for (unsigned int tracksterIndex = 0; tracksterIndex < simTracksters.size(); ++tracksterIndex) {
    ...
    ticl::mapWithFraction hitToAssociatedRecoTracksterMap(simTracksterHitsAndFractions.size());
    ...
    for (unsigned int i = 0; i < simTracksterHitsAndFractions.size(); ++i) {
      ...
      for (const auto& [recoTracksterIndex, recoFraction] : hitToRecoTracksterVec) {
        hitToAssociatedRecoTracksterMap[i].emplace_back(recoTracksterIndex, recoFraction);
    ...
    for (unsigned int i = 0; i < simTracksterHitsAndFractions.size(); ++i) {
      ...
      for (unsigned int recoTracksterIndex : associatedRecoTracksterIndices) {
        if (std::find_if(hitToRecoTracksterVec.begin(), hitToRecoTracksterVec.end(), [recoTracksterIndex](const auto& pair) { return pair.first == recoTracksterIndex;}) == hitToRecoTracksterVec.end()) {
          hitToAssociatedRecoTracksterMap[i].emplace_back(recoTracksterIndex, 0);
        }
      }
    }
    ...
    for (unsigned int i = 0; i < simTracksterHitsAndFractions.size(); ++i) {
      ...
      for (const auto& [recoTracksterIndex, recoFraction] : hitToRecoTracksterVec) {
        ...
        float score = invDenominator * squaredSimFraction * squaredRecHitEnergy;
