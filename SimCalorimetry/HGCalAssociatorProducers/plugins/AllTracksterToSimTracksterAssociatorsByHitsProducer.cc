...
AllTracksterToSimTracksterAssociatorsByHitsProducer::AllTracksterToSimTracksterAssociatorsByHitsProducer(const edm::ParameterSet& pset) :
          ...
          pset.getParameter<edm::InputTag>("hitToCaloParticleMap")))
{
  ...
  for (const auto& tag : tracksterCollections) {
    ...
    hitToTracksterMapTokens_.emplace_back(label, consumes<ticl::AssociationMap<ticl::mapWithFraction>>(edm::InputTag("allHitToTracksterAssociations", "hitTo"+label)));
    tracksterToHitMapTokens_.emplace_back(label, consumes<ticl::AssociationMap<ticl::mapWithFraction>>(edm::InputTag("allHitToTracksterAssociations", label+"ToHit")));
  }
  ...
  for (const auto& tag : simTracksterCollections) {
    ...
    hitToSimTracksterMapTokens_.emplace_back(label, consumes<ticl::AssociationMap<ticl::mapWithFraction>>(edm::InputTag("allHitToTracksterAssociations", "hitTo"+label)));
    simTracksterToHitMapTokens_.emplace_back(label, consumes<ticl::AssociationMap<ticl::mapWithFraction>>(edm::InputTag("allHitToTracksterAssociations", label+"ToHit")));
  }
  ...
  for (const auto& tracksterToken : tracksterCollectionTokens_) {
    for (const auto& simTracksterToken : simTracksterCollectionTokens_) {
      ...
      produces<ticl::AssociationMap<ticl::mapWithFractionAndScore, std::vector<ticl::Trackster>, std::vector<ticl::Trackster>>>(instanceLabel);
      std::string reverseInstanceLabel = simTracksterToken.first + "To" + tracksterToken.first;
      produces<ticl::AssociationMap<ticl::mapWithFractionAndScore, std::vector<ticl::Trackster>, std::vector<ticl::Trackster>>>(reverseInstanceLabel);
    }
  }
}

void AllTracksterToSimTracksterAssociatorsByHitsProducer::produce(edm::StreamID, edm::Event& iEvent, const edm::EventSetup&) const {
  using namespace edm;
  ...
  for (const auto& tracksterToken : tracksterCollectionTokens_) {
    ...
    auto tracksterMapTokenIter = std::find_if(hitToTracksterMapTokens_.begin(), hitToTracksterMapTokens_.end(),
    ...
    auto tracksterToHitMapTokenIter = std::find_if(tracksterToHitMapTokens_.begin(), tracksterToHitMapTokens_.end(),
                                                   [&tracksterToken](const auto& pair) { return pair.first == tracksterToken.first; });
    ...
    for (const auto& simTracksterToken : simTracksterCollectionTokens_) {
      ...
      auto simTracksterMapTokenIter = std::find_if(hitToSimTracksterMapTokens_.begin(), hitToSimTracksterMapTokens_.end(),
      ...
      // Retrieve the correct SimTracksterToHitMap for the current simTrackster collection
        ...
        auto simTracksterToHitMapTokenIter = std::find_if(simTracksterToHitMapTokens_.begin(), simTracksterToHitMapTokens_.end(),
                                                        [&simTracksterToken](const auto& pair) { return pair.first == simTracksterToken.first; });
...
void AllTracksterToSimTracksterAssociatorsByHitsProducer::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
  edm::ParameterSetDescription desc;
  desc.add<std::vector<edm::InputTag>>("tracksterCollections", {edm::InputTag("ticlTrackstersCLUE3DHigh"), edm::InputTag("ticlTrackstersLinks")});
  desc.add<std::vector<edm::InputTag>>("simTracksterCollections", {edm::InputTag("ticlSimTracksters"), edm::InputTag("ticlSimTracksters", "fromCPs")});
