...
AllTracksterToSimTracksterAssociatorsByLCsProducer::AllTracksterToSimTracksterAssociatorsByLCsProducer(const edm::ParameterSet& pset)
  ...
  for (const auto& tag : tracksterCollections) {
    ...
    layerClusterToTracksterMapTokens_.emplace_back(label, consumes<ticl::AssociationMap<ticl::mapWithFraction, std::vector<reco::CaloCluster>, std::vector<ticl::Trackster>>>(edm::InputTag("allLayerClusterToTracksterAssociations", label)));
  }
  ...
  for (const auto& tag : simTracksterCollections) {
    ...
    layerClusterToSimTracksterMapTokens_.emplace_back(label, consumes<ticl::AssociationMap<ticl::mapWithFraction, std::vector<reco::CaloCluster>, std::vector<ticl::Trackster>>>(edm::InputTag("allLayerClusterToTracksterAssociations", label)));
  }
...
void AllTracksterToSimTracksterAssociatorsByLCsProducer::produce(edm::StreamID, edm::Event& iEvent, const edm::EventSetup&) const {
  using namespace edm;
  ...
  for (const auto& tracksterToken : tracksterCollectionTokens_) {
    ...
    Handle<ticl::AssociationMap<ticl::mapWithFraction, std::vector<reco::CaloCluster>, std::vector<ticl::Trackster>>> layerClusterToTracksterMapHandle;
    auto tracksterMapTokenIter = std::find_if(layerClusterToTracksterMapTokens_.begin(), layerClusterToTracksterMapTokens_.end(),
    ...
    for (const auto& simTracksterToken : simTracksterCollectionTokens_) {
      ...
      Handle<ticl::AssociationMap<ticl::mapWithFraction, std::vector<reco::CaloCluster>, std::vector<ticl::Trackster>>> layerClusterToSimTracksterMapHandle;
      auto simTracksterMapTokenIter = std::find_if(layerClusterToSimTracksterMapTokens_.begin(), layerClusterToSimTracksterMapTokens_.end(),
 ...
 for (unsigned int tracksterIndex = 0; tracksterIndex < simTracksters.size(); ++tracksterIndex) {
  ...
  simTracksterToTracksterMap->sort(true);
      ...
      std::cout << "Producing " << tracksterToken.first + "To" + simTracksterToken.first << std::endl;
      iEvent.put(std::move(simTracksterToTracksterMap), simTracksterToken.first + "To" + tracksterToken.first);
      std::cout << "Producing " << simTracksterToken.first + "To" + tracksterToken.first << std::endl;
    }
  }
}
