...
#include "CondFormats/HGCalObjects/interface/HGCalGeomHostCollection.h"
...
struct TICLGeom {
  ...
  std::unique_ptr<HGCalGeomHostCollection> hostCollection;

  TICLGeom() = delete;
  explicit TICLGeom(std::size_t size)
      : hostCollection(std::make_unique<HGCalGeomHostCollection>(size, cms::alpakatools::host())) {}
