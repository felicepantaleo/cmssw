...
TICLPFBlockAlgo::TICLPFBlockAlgo()
    : debug_(false),
...
void TICLPFBlockAlgo::setLinkers(const std::vector<edm::ParameterSet>& confs) {
  ...
  const std::string pfx_kdtree("Tiles");
  for (const auto& conf : confs) {
    ...
    if (useKDTree) {
      kdtrees_.emplace_back(TICLTilesLinkerFactory::get()->create(pfx_kdtree + linkerName, conf));
...
void TICLPFBlockAlgo::setImporters(const std::vector<edm::ParameterSet>& confs, edm::ConsumesCollector& cc) {
...
#ifdef PFLOW_DEBUG
  if (debug_)
    cout << "~TICLPFBlockAlgo - number of remaining elements: " << elements_.size() << endl;
#endif
}
...
inline void TICLPFBlockAlgo::link(const reco::PFBlockElement* el1,
  ...
  const unsigned index = rowsize * std::max(type1, type2) + std::min(type1, type2);
  if (debug_) {
    std::cout << " TICLPFBlockAlgo links type1 " << type1 << " type2 " << type2 << std::endl;
  }
...
void TICLPFBlockAlgo::updateEventSetup(const edm::EventSetup& es) {
...
void TICLPFBlockAlgo::buildElements(const edm::Event& evt) {
...
std::ostream& operator<<(std::ostream& out, const TICLPFBlockAlgo& a) {
  if (!out)
    return out;
