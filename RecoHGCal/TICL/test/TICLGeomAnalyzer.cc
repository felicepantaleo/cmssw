...
void TICLGeomAnalyzer::analyze(const edm::Event&, const edm::EventSetup& iSetup) {
  ...
  for (const auto& [detId, index] : ticlGeom.detIdToIndexMap) {
    std::cout << label_ << "\t DetId: " << detId << "\tIndex: " << index << "\tx: " << ticlGeomView.x()[index]
              << "\ty: " << ticlGeomView.y()[index] << "\tz: " << ticlGeomView.z()[index]
              << "\teta: " << ticlGeomView.eta()[index] << "\tphi " << ticlGeomView.phi()[index] << std::endl;
  }
}
