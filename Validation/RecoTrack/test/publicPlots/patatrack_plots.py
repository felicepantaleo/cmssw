...
def main():
    ...
    styleReference = lambda h: applyStyle(h, ROOT.kBlue, 21)
    styleCompare1 = lambda h: applyStyle(h, ROOT.kRed, 20)
    styleCompare2 = lambda h: applyStyle(h, ROOT.kBlack, 22)
    ...
    referenceName = "referenceWF"
    compare1Name = "compare1WF"
    compare2Name = "compare2WF"
    reference = (
    ...
    filesCombination1 = Files([reference, compare1])
    filesCombination2 = Files([reference, compare2])
    filesCombination3 = Files([compare1, compare2])



    pileup = "#LTPU#GT=50"

    plotEffAndFake(filesCombination2, "ttbar_pu50_"+referenceName+"_"+compare1Name+"", pileup)
    plotEffAndFake(filesCombination1, "ttbar_pu50_"+referenceName+"_"+compare2Name+"", pileup)
    plotEffAndFake(filesCombination3, "ttbar_pu50_"+compare1Name+"_"+compare2Name+"", pileup)

    plotResol(filesCombination2, "ttbar_pu50_"+referenceName+"_"+compare1Name+"", pileup)
    plotResol(filesCombination2, "ttbar_pu50_"+referenceName+"_"+compare1Name+"", pileup, ptCut09=True)

    plotResol(filesCombination3, "ttbar_pu50_"+compare1Name+"_"+compare2Name+"", pileup)
    plotResol(filesCombination3, "ttbar_pu50_"+compare1Name+"_"+compare2Name+"", pileup, ptCut09=True)

    plotVertex(filesCombination2, "ttbar_pu50_"+referenceName+"_"+compare1Name+"", pileup)
    plotVertex(filesCombination3, "ttbar_pu50_"+compare1Name+"_"+compare2Name+"", pileup)
...
################################################################################
################################################################################
def plotEffAndFake(files, prefix, pileup, hasPU70=False):
    folder_eff = "Tracking/Run summary/PixelTrack/pixel_trackingParticlePixelAsssociation/"
    folder_fake = "Tracking/Run summary/PixelTrack/pixelPt09_trackingParticlePixelAsssociation/"
...
################################################################################
################################################################################
def plotColoredEff(compare1file, prefix, pileup):
    #folder_track = "DQMData/Run 1/Tracking/Run summary/PixelTrack/cutsReco%s_trackingParticleRecoAsssociation/"
    #folder_track = "DQMData/Run 1/Tracking/Run summary/PixelTrack/cutsReco%sHp_trackingParticleRecoAsssociation/"
    folder_track = "DQMData/Run 1/Tracking/Run summary/PixelTrack/cutsReco%sByOriginalAlgoHp_trackingParticleRecoAsssociation/"
    iterations = [
        "InitialStep",
        "HighPtTripletStep",
        "LowPtQuadStep",
...
#    effbox_pt.move(dx=-0.13, dy=0.6)
    ...
    for it, leg in zip(iterations, legendLabels):
        ...
        h = compare1file.Get(hname)
        if not h:
            raise Exception("Did not find %s from %s" % (hname, compare1file.GetName()))
...
#        "legendTransparent": True,
    ...
    for it, leg in zip(iterations, legendLabels):
        ...
        eff = calculateEfficiency(compare1file, num_name, denom_name, rebin=2)
...
def plotFake(files, prefix, pileup):
    folder_track = "Tracking/Run summary/PixelTrack/"
...
################################################################################
################################################################################
def plotResol(files, prefix, pileup, hasPU70=False, ptCut09 = False):
    folder_track = "Tracking/Run summary/PixelTrack/pixel_trackingParticlePixelAsssociation/"
    if ptCut09:
        folder_track = "Tracking/Run summary/PixelTrack/pixelPt09_trackingParticlePixelAsssociation/"
        prefix = prefix + "_pt09"

    #folder_vertex = "Vertexing/Run summary/PrimaryVertexV/offlinePrimaryVertices/"
    folder_vertex = "Vertexing/Run summary/PrimaryVertexV/selectedPixelVertices/"
...
def printEffFake(files, pileup):
    ...
    for f, l in zip(files.getFiles(), files.getLegends()):
        eff_h = f.Get("DQMData/Run 1/Tracking/Run summary/PixelTrack/effic_vs_coll")
        fake_h = f.Get("DQMData/Run 1/Tracking/Run summary/PixelTrack/fakerate_vs_coll")
...
#        ly1 = 0.8
    ...
    canv.SaveAs("/afs/cern.ch/user/f/fpantale/www/CTD19/publicPlots/"+name+".png")
    canv.SaveAs("/afs/cern.ch/user/f/fpantale/www/CTD19/publicPlots/"+name+".pdf")
