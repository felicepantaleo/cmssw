...
public:
  ...
  void analyze(edm::Event const& e, edm::EventSetup const&) override {
    ...
    for (uint32_t i = 0; i < g.nParticles(); ++i) {
      ...
      if (!pd.hasSim() || pd.simNode < 0 || uint32_t(pd.simNode) >= nN) continue;
      if (rg.nodeRef(uint32_t(pd.simNode)).kind != TruthGraph::NodeKind::SimTrack) continue;
    ...
    for (uint32_t n = 0; n < nN; ++n) {
      ...
      if (ref.kind != TruthGraph::NodeKind::SimTrack) continue;
      if (ref.key > 0) keyToNode[key(rg.nodeEventId(n), uint32_t(ref.key))] = n;
      for (uint32_t c : rg.children(n))
        if (c < nN && rg.nodeRef(c).kind == TruthGraph::NodeKind::SimVertex) vertexParent[c] = int32_t(n);
    }
    // nearest-ancestor particle (the NEW roll-up), memoized
    std::vector<int32_t> owner(nN, -2);
    auto resolve = [&](uint32_t start) -> int32_t {
      std::vector<uint32_t> chain; int32_t o = -1; uint32_t cur = start;
      for (uint32_t guard = 0; guard <= nN; ++guard) {
        if (owner[cur] != -2) { o = owner[cur]; break; }
        auto it = simNodeToParticle.find(cur);
        if (it != simNodeToParticle.end()) { o = int32_t(it->second); break; }
        chain.push_back(cur);
        int32_t v = rg.nodeSimTrackToVtx(cur);
        if (v < 0 || uint32_t(v) >= nN || vertexParent[uint32_t(v)] < 0) { o = -1; break; }
        cur = uint32_t(vertexParent[uint32_t(v)]);
      }
      for (uint32_t c : chain) owner[c] = o;
      return o;
    };
    ...
    for (auto const& h : hits) {
      int gt = h.geantTrackId(); if (gt <= 0) continue;
      uint64_t eid = h.eventId().rawId(); uint64_t k = key(eid, uint32_t(gt));
      double en = h.energy();
      ...
      if (eid == 0) { ++sigH; sigE += en; if (old){++sigHold;sigEold+=en;} if (neu){++sigHnew;sigEnew+=en;} }
      else          { ++puH;  puE += en;  if (old){++puHold; puEold+=en;}  if (neu){++puHnew; puEnew+=en;} }
    }
    ...
    for (uint32_t n = 0; n < nN; ++n) {
      auto const& ref = rg.nodeRef(n);
      if (ref.kind != TruthGraph::NodeKind::SimTrack) continue;
      if (rg.nodeEventId(n) == 0ull) continue;  // PU only
      ++puNodes;
      int32_t v = rg.nodeSimTrackToVtx(n);
      if (v >= 0 && uint32_t(v) < nN) { ++puWithVtx; if (vertexParent[uint32_t(v)] >= 0) ++puWithParent; }
      if (resolve(n) >= 0) ++puReachPart;
      if (rg.nodeSimTrackToGen(n) >= 0) ++puGenLinked;
    }
